...
p07 <- read.csv(file = 'moy_tot_quantif_2007.csv', sep=';', dec = ',', fileEncoding = 'latin1')
p08 <- read.csv(file = 'moy_tot_quantif_2008.csv', sep=';', dec = ',', fileEncoding = 'latin1')
p09 <- read.csv(file = 'moy_tot_quantif_2009.csv', sep=';', dec = ',', fileEncoding = 'latin1')
p10 <- read.csv(file = 'moy_tot_quantif_2010.csv', sep=';', dec = ',', fileEncoding = 'latin1')
p11 <- read.csv(file = 'moy_tot_quantif_2011.csv', sep=';', dec = ',', fileEncoding = 'latin1')
p12 <- read.csv(file = 'moy_tot_quantif_2012.csv', sep=';', dec = ',', fileEncoding = 'latin1')
p13 <- read.csv(file = 'moy_tot_quantif_2013.csv', sep=';', dec = ',', fileEncoding = 'latin1')
p14 <- read.csv(file = 'moy_tot_quantif_2014.csv', sep=';', dec = ',', fileEncoding = 'latin1')
...
stations_sp@proj4string <- CRS("+init=epsg:2154") # la couche est projetée en Lambert93
...
stations_sp@data <- stations_sp@data %>% nest(-c(CD_STATION, NOM_COM, NUM_DEP, PROFONDEUR_MAXI_POINT, 
                                                 CD_ME_v2, CD_ME_niv1_surf, 
                                                 moy2007, moy2008, moy2009, moy2010, moy2011, moy2012, moy2013, moy2014,
                                           selection, index))
stations_sp@data <- stations_sp@data %>% rename( nbprel = data ) %>% mutate(selection = F)
stations_sp@data$nbprel <- lapply(stations_sp@data$nbprel, function(x) melt(x, measure.vars = paste0("prel", 2007:2014)))
